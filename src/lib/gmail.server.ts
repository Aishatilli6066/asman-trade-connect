...
function buildRaw(opts: { to: string; subject: string; html: string; replyTo?: string }) {
  const lines = [
    `To: ${opts.to}`,
...
export async function sendGmail(opts: { to: string; subject: string; html: string; replyTo?: string }) {
  const lovableKey = process.env.LOVABLE_API_KEY;
  const gmailKey = process.env.GOOGLE_MAIL_API_KEY;
  if (!lovableKey || !gmailKey) {
    console.error("Gmail send skipped: missing credentials");
